...
// advice message for notifications
let title_for_message = "Warning";
...
const surveyURL = 'https://sligo.up.railway.app/newcall';
const welcomeImg = '../images/Sligo.png';
...
let mirotalkBtnsBar = 'horizontal'; // vertical - horizontal
...
function getHtmlElementsById() {
    ...
    secureRoomBtn = getId('secureRoomBtn');
    // chat Room elements
...
function welcomeUser() {
    ...
    Swal.fire({
        background: swalBackground,
        position: 'center',
        title: '<strong>Welcome to Sligo, ' + myPeerName + '</strong>',
        imageAlt: 'sligo-welcome',
        imageUrl: welcomeImg,
        html:
            `
        <br/> 
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Join this Sligo Chat Meeting',
...
function setTheme(theme) {
    ...
    switch (mirotalkTheme) {
        ...
        case 'blue':
            // blue theme
            swalBackground = 'radial-gradient(#4879B2, #3417B7)';
            document.documentElement.style.setProperty('--body-bg', 'radial-gradient(#4879B2, #3417B7)');
            document.documentElement.style.setProperty('--msger-bg', 'radial-gradient(#4879B2, #3417B7)');
            document.documentElement.style.setProperty('--wb-bg', 'radial-gradient(#797979, #000)');
            document.documentElement.style.setProperty('--box-shadow', '0px 8px 16px 0px rgba(0, 0, 0, 0.2)');
            document.documentElement.style.setProperty('--msger-private-bg', 'radial-gradient(#4879B2, #3417B7)');
            document.documentElement.style.setProperty('--left-msg-bg', '#222328');
            document.documentElement.style.setProperty('--private-msg-bg', '#f77070');
            document.documentElement.style.setProperty('--right-msg-bg', '#0a0b0c');
            break;
        case 'sunrise':
            // blue theme
            swalBackground = 'radial-gradient(#2DE3B2, #DE7584 )';
            document.documentElement.style.setProperty('--body-bg', 'radial-gradient(#2DE3B2, #DE7584)');
            document.documentElement.style.setProperty('--msger-bg', 'radial-gradient(#2DE3B2, #DE7584)');
            document.documentElement.style.setProperty('--wb-bg', 'radial-gradient(#797979, #000)');
            document.documentElement.style.setProperty('--box-shadow', '0px 8px 16px 0px rgba(0, 0, 0, 0.2)');
            document.documentElement.style.setProperty('--msger-private-bg', 'radial-gradient(#2DE3B2, #DE7584)');
            document.documentElement.style.setProperty('--left-msg-bg', '#222328');
            document.documentElement.style.setProperty('--private-msg-bg', '#f77070');
            document.documentElement.style.setProperty('--right-msg-bg', '#0a0b0c');
            break;              
...
function handleVideoPlayerFs(videoId, videoFullScreenBtnId, peer_id = null) {
    ...
    function showMsg() {
        userLog('toast', 'Full screen mode is disabled when video is not on');
    }
...
function setChatRoomBtn() {
    ...
    msgerCPBtn.addEventListener('click', (e) => {
        if (!thereIsPeerConnections()) {
            userLog(title_for_message, 'No participants detected');
            return;
        }
    ...
    msgerClean.addEventListener('click', (e) => {
        ...
        userLog(title_for_message, 'No chat messages to delete');
    });
    ...
    msgerSaveBtn.addEventListener('click', (e) => {
        ...
        userLog(title_for_message, 'No chat messages to save');
    });
...
function setCaptionRoomBtn() {
    if (window.SpeechRecognition || window.webkitSpeechRecognition) {
        ...
        captionClean.addEventListener('click', (e) => {
            ...
            userLog(title_for_message, 'No captions to delete');
        });
        ...
        captionSaveBtn.addEventListener('click', (e) => {
            ...
            userLog(title_for_message, 'No captions to save');
        });
    ...
    if (!isSupportedNavigatorShare || (isSupportedNavigatorShare && errorNavigatorShare)) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let message = {
                    email: '',
                    subject: 'Join our SECURE Sligo meeting',
                    body: 'Click to join: ' + myRoomUrl,
                };
                shareRoomByEmail(message);
            }
...
function sendChatMessage() {
    if (!thereIsPeerConnections()) {
        userLog(title_for_message, "Can't send message, no participants in the room");
        msgerInput.value = '';
        return;
    }
